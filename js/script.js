...
class CannonHelper {
  ...
  addLights(renderer) {
    ...
    const light = new THREE.DirectionalLight(0xdddddd, 0.25, 1);
      ...
      switch (shape.type) {
        ...
        case CANNON.Shape.types.PLANE:
          ...
          var floorMap = THREE.ImageUtils.loadTexture(
            'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQMaznkwKbnlWTf0zzL9uQrUQ2Q54MfyI7JC5m62icHR5oRjT1v'
          );
...
Promise.all([
  ...
  terrainBody.position.set(
    (-sizeX * terrainShape.elementSize) / 2, -10,
    (sizeY * terrainShape.elementSize) / 2
  );
  ...
  raycastHelperMesh = new THREE.Mesh(
  ...
  check = function() {
    ...
    var intersects = raycaster.intersectObject(
      terrainBody.threemesh.children[0]
    );
    ...
    mesh.position.y =
      intersects && intersects[0] ? intersects[0].point.y + 0.1 : 30;
    ...
    var intersects2 = raycaster2.intersectObject(
      terrainBody.threemesh.children[0]
    );

    //position objects ontop of the terrain
    flagLocation.position.y =
      intersects2 && intersects2[0] ? intersects2[0].point.y + 0.5 : 30;
...
var texture = new THREE.TextureLoader().load(
  'https://pbs.twimg.com/profile_images/1574215892174602241/7OcurLzE_400x400.jpg'
);
...
loader.load(
  'https://raw.githubusercontent.com/baronwatts/models/master/moon-vehicle.js',
...
(function animate() {
  ...
  //display coordinates
  info.innerHTML = `<span>X: </span>${mesh.position.x.toFixed(
		2
	)}, &nbsp;&nbsp;&nbsp; <span>Y: </span>${mesh.position.y.toFixed(
		2
	)}, &nbsp;&nbsp;&nbsp; <span>Z: </span>${mesh.position.z.toFixed(2)}`;
